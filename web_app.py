...
# Kleiner CSS-Hack, um Streamlit noch etwas enger zu machen
st.markdown("""
    <style>
...
# ==========================================
# 2. SEITENLEISTE (Sidebar) FÜR LEVEL CAPS
# ==========================================
LEVEL_CAPS = {
    "Gen 1: Rot/Blau": [
...
# ==========================================
st.title("🔥 Pokémon Soul-Link Tracker")
        ...
        if not route_input or not p_pokes[0]:
            ...
            with st.spinner('Prüfe Datenbank...'):
                erfolg, nachricht = tracker_logik.link_hinzufuegen(
                    route_input, p_names[0], p_pokes[0], p_names[1], p_pokes[1], p_names[2], p_pokes[2], status_input
                )
                if erfolg:
...
def zeichne_zeile(eintrag, zeige_checkbox=False):
    ...
    with st.container(border=True):
        # 1. Zeile: ALLES reinquetschen (Route, Team-Checkbox, Buttons)
        ...
        with top_cols[1]:
            if zeige_checkbox:
                ...
                if neu_in_team != in_team:
                    erfolg, msg = tracker_logik.team_status_aendern(link_id, neu_in_team)
        ...
        # 2. Zeile: Die Pokémon-Karten (Enger und kleiner)
            ...
            if pokemon_name:
                ...
                with karten_cols[idx]:
                    ...
                    with sub_col1:
                        ...
                        # ---> HIER IST DIE ÄNDERUNG FÜR DIE INTERNET-BILDER <---
                        if bild_pfad: 
...
# 5. TABS ZEICHNEN
# ==========================================
daten = tracker_logik.lade_daten()
