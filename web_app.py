...
# Hier tippt man den Namen des Runs ein:
aktueller_run = st.sidebar.text_input("Name des Runs:", value="Standard_Run")

st.sidebar.markdown("---")

LEVEL_CAPS = {
    "Gen 1: Rot/Blau": ["1. Arena: Lv 14", "2. Arena: Lv 21", "3. Arena: Lv 24", "4. Arena: Lv 29", "5. Arena: Lv 43", "6. Arena: Lv 43", "7. Arena: Lv 47", "8. Arena: Lv 50", "Top 4: Lv 56", "Champ: Lv 65"],
    "Gen 2: Gold/Silber": ["1. Arena: Lv 9", "2. Arena: Lv 16", "3. Arena: Lv 20", "4. Arena: Lv 25", "5. Arena: Lv 30", "6. Arena: Lv 35", "7. Arena: Lv 31", "8. Arena: Lv 40", "Top 4: Lv 42", "Champ: Lv 50"],
    "Gen 3: Smaragd": ["1. Arena: Lv 15", "2. Arena: Lv 19", "3. Arena: Lv 24", "4. Arena: Lv 29", "5. Arena: Lv 31", "6. Arena: Lv 33", "7. Arena: Lv 42", "8. Arena: Lv 46", "Top 4: Lv 49", "Champ: Lv 58"],
    "Gen 4: Platin": ["1. Arena: Lv 14", "2. Arena: Lv 22", "3. Arena: Lv 26", "4. Arena: Lv 32", "5. Arena: Lv 37", "6. Arena: Lv 41", "7. Arena: Lv 44", "8. Arena: Lv 50", "Top 4: Lv 53", "Champ: Lv 62"],
    "Gen 5: Schwarz/Weiß": ["1. Arena: Lv 14", "2. Arena: Lv 20", "3. Arena: Lv 23", "4. Arena: Lv 27", "5. Arena: Lv 31", "6. Arena: Lv 35", "7. Arena: Lv 39", "8. Arena: Lv 43", "Liga: Lv 50", "Champ: Lv 77"]
}
...
def zeichne_zeile(eintrag, zeige_checkbox=False):
    ...
    with st.container(border=True):
        ...
        with top_cols[2]:
            if status == "aktiv":
                if st.button("🏃", key=f"v_{link_id}"): tracker_logik.status_aendern(link_id, "verpasst"); st.rerun()
            else:
                if st.button("♻️", key=f"a_{link_id}"): tracker_logik.status_aendern(link_id, "aktiv"); st.rerun()
        with top_cols[3]:
            if status != "friedhof":
                if st.button("💀", key=f"f_{link_id}"): tracker_logik.status_aendern(link_id, "friedhof"); st.rerun()
            else:
                if st.button("🏃", key=f"v2_{link_id}"): tracker_logik.status_aendern(link_id, "verpasst"); st.rerun()
        with top_cols[4]:
            if st.button("🗑️", key=f"d_{link_id}"): tracker_logik.link_loeschen(link_id); st.rerun()
...
# 5. TABS ZEICHNEN
# ==========================================
# Daten werden nun spezifisch für den EINGETIPPTEN Run geladen!
